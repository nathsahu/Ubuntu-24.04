...
requirements() {
    echo ${C}"Installing ubuntu-lts using proot-distro"
    sleep 1 
    echo ${C}"Installing required packages..."${W}
    ...
    [[ -d "$PD/$ds_name" ]] && {
        if ask ${R}"Existing ubuntu found, remove it?"${W}; then
            echo ""
            echo ${G}"Deleting existing directory...."${W}
...
choose_desktop() {
    ...
    echo " 2) Windows 10 (KDE with custom themes)"
    echo ${Y}"Please enter number 1 or 2 to choose your desktop "
    echo ${W}"If you don't want a desktop please just enter '${W}CLI${C}'"${W}
    read desktop
    sleep 1
    case $desktop in
        1) echo ${G}"Lets start the installation"${Y} ;;
        2) echo ${Y}"Gnome is no longer supported..."${Y} ; sleep 2 ; choose_desktop;;
...
configures() {
    ...
    echo ${C}"Installing requirements in ubuntu-lts..."${W}
...
user() {
    clear
    if ask ${G}"Do you want to add a user ?"${W}; then
        echo ""
...
xfce_mode() {
    echo ${C}"Installing XFCE Desktop..."${W}
...
apps() {
    ...
    # Install firefox
    if ask ${G}"Install Firefox Web Broswer?"${W}; then
...
EOF
    ...
    if ask ${G}"Install Discord (Webcord)?"${W}; then
    ...
    # Install VScode
    if ask ${G}"Install VScode?"${W}; then
        echo -e ${Y}"\nInstalling Vscode ...." ${W}
